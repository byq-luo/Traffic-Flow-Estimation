...
'''
data = data[data.index.month == MAE]
...
data = bm.join_hour_one_hot(data)
...
indexes = bm.find_indexes_of_month(data, FEB)
#indexes.extend(bm.find_indexes_of_month(data, FEB))
indexes.extend(bm.find_indexes_of_month(data, MAR))
indexes.extend(bm.find_indexes_of_month(data, APR))
indexes.extend(bm.find_indexes_of_month(data, MAY))
indexes.extend(bm.find_indexes_of_month(data, OCT))
indexes.extend(bm.find_indexes_of_month(data, NOV))
x_train, y_train = bm.build_sets(data, indexes, DISTANCE, TIME_BACK,
                                 TIME_FORWARD, SAMPLE_FREQUENCY)
indexes = bm.find_indexes_of_month(data, DEC)
